...
class MinHeap:
    ...
    def pop(self) -> Node:
        ...
        while child_idx < self.size():
            if (
                child_idx + 1 < self.size()
                and self.arr[child_idx + 1] < self.arr[child_idx]
            ):
                child_idx += 1
...
def decode(
    ...
    if check_stats:
        ...
        for c, f in freqs.items():
            if stats[c] != f:
                raise ValueError(
                    f"incorrect '{c}' freq: header={f} processed={stats[c]}, {unpacked=}"
                )
